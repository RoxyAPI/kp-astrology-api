...
Five details are worth copying:
...
- `areas.finance.composite` on `daily` is an equal weight of the day score, the finance score and the natal wealth count, printed with its three layers so the division can be redone or reweighted. It is null when the running lords reach none of the six finance houses, which is an absence of connection rather than a weak verdict.
...
<roxy-vedic-daily id="daily"></roxy-vedic-daily>

<script type="module">
  // chart, dasha, ruling and reading are the responses daily-flow.ts already fetched
  ...
  document.getElementById('daily').data = reading;
</script>
```
